...
#define ETH_BPGEN_IFACE_ARG		"iface"
#define ETH_BPGEN_NUM_Q_ARG		"qpairs"
#define ETH_BPGEN_BLOCKSIZE_ARG	"blocksz"
#define ETH_BPGEN_FRAMESIZE_ARG	"framesz"
#define ETH_BPGEN_FRAMECOUNT_ARG	"framecnt"
#define ETH_BPGEN_QDISC_BYPASS_ARG	"qdisc_bypass"

#define DFLT_BLOCK_SIZE		(1 << 12)
#define DFLT_FRAME_SIZE		(1 << 11)
#define DFLT_FRAME_COUNT	(1 << 9)

#define RTE_PMD_BPGEN_MAX_RINGS 16
...
struct pmd_internals {
	...
	struct pkt_rx_queue rx_queue[RTE_PMD_BPGEN_MAX_RINGS];
	struct pkt_tx_queue tx_queue[RTE_PMD_BPGEN_MAX_RINGS];
};

static const char *valid_arguments[] = {
	ETH_BPGEN_IFACE_ARG,
	ETH_BPGEN_NUM_Q_ARG,
	ETH_BPGEN_BLOCKSIZE_ARG,
	ETH_BPGEN_FRAMESIZE_ARG,
	ETH_BPGEN_FRAMECOUNT_ARG,
	ETH_BPGEN_QDISC_BYPASS_ARG,
	NULL
};
...
static int bpgen_logtype;

#define PMD_LOG(level, fmt, args...) \
	rte_log(RTE_LOG_ ## level, bpgen_logtype, \
		"%s(): " fmt "\n", __func__, ##args)

static uint16_t
eth_bpgen_rx(void *queue, struct rte_mbuf **bufs, uint16_t nb_pkts)
{
	unsigned i;
	...
	/*
	 * Reads the given number of packets from the BPGEN socket one by
 ...
 */
static uint16_t
eth_bpgen_tx(void *queue, struct rte_mbuf **bufs, uint16_t nb_pkts)
{
	struct tpacket2_hdr *ppd;
	struct rte_mbuf *mbuf;
	uint8_t *pbuf;
                  ...
                  void *extra_args)
{
	...
	/* Open an BPGEN socket... */
	*sockfd = socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
	if (*sockfd == -1) {
		PMD_LOG(ERR, "Could not open BPGEN socket");
		return -1;
	}
...
static struct rte_vdev_driver pmd_bpgen_drv;
                       ...
                       struct rte_kvargs *kvlist)
{
	...
	for (k_idx = 0; k_idx < kvlist->count; k_idx++) {
		...
		if (strstr(pair->key, ETH_BPGEN_IFACE_ARG) != NULL)
			break;
	}
	if (pair == NULL) {
		PMD_LOG(ERR,
			"%s: no interface specified for BPGEN ethdev",
		        name);
		return -1;
	}

	PMD_LOG(INFO,
		"%s: creating BPGEN-backed ethdev on numa socket %u",
		name, numa_node);
	...
	*internals = rte_zmalloc_socket(name, sizeof(**internals),
	...
	for (q = 0; q < nb_queues; q++) {
		/* Open an BPGEN socket for this queue... */
		qsockfd = socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
		if (qsockfd == -1) {
			PMD_LOG(ERR,
				"%s: could not open BPGEN socket",
			        name);
			return -1;
		}
		...
		if (rc == -1) {
			PMD_LOG(ERR,
				"%s: could not set PACKET_VERSION on BPGEN socket for %s",
				name, pair->value);
			goto error;
		}
		...
		if (rc == -1) {
			PMD_LOG(ERR,
				"%s: could not set PACKET_LOSS on BPGEN socket for %s",
				name, pair->value);
			goto error;
		}
		...
		if (rc == -1) {
			PMD_LOG(ERR,
				"%s: could not set PACKET_QDISC_BYPASS on BPGEN socket for %s",
				name, pair->value);
			goto error;
		}
#else
		...
		if (rc == -1) {
			PMD_LOG(ERR,
				"%s: could not set PACKET_RX_RING on BPGEN socket for %s",
				name, pair->value);
			goto error;
		}
		...
		if (rc == -1) {
			PMD_LOG(ERR,
				"%s: could not set PACKET_TX_RING on BPGEN "
		...
		if (rx_queue->map == MAP_FAILED) {
			PMD_LOG(ERR,
				"%s: call to mmap failed on BPGEN socket for %s",
				name, pair->value);
			goto error;
		}
		...
		if (rc == -1) {
			PMD_LOG(ERR,
				"%s: could not bind BPGEN socket to %s",
			        name, pair->value);
			goto error;
		}
		...
		if (rc == -1) {
			PMD_LOG(ERR,
				"%s: could not set PACKET_FANOUT on BPGEN socket "
                    ...
                    struct rte_kvargs *kvlist)
{
	...
	for (k_idx = 0; k_idx < kvlist->count; k_idx++) {
		pair = &kvlist->pairs[k_idx];
		if (strstr(pair->key, ETH_BPGEN_NUM_Q_ARG) != NULL) {
			qpairs = atoi(pair->value);
			if (qpairs < 1 ||
			    qpairs > RTE_PMD_BPGEN_MAX_RINGS) {
				PMD_LOG(ERR,
		...
		if (strstr(pair->key, ETH_BPGEN_BLOCKSIZE_ARG) != NULL) {
		...
		if (strstr(pair->key, ETH_BPGEN_FRAMESIZE_ARG) != NULL) {
		...
		if (strstr(pair->key, ETH_BPGEN_FRAMECOUNT_ARG) != NULL) {
		...
		if (strstr(pair->key, ETH_BPGEN_QDISC_BYPASS_ARG) != NULL) {
	...
	if (framesize > blocksize) {
		PMD_LOG(ERR,
			"%s: BPGEN MMAP frame size exceeds block size!",
		        name);
		return -1;
	}
	...
	if (!blockcount) {
		PMD_LOG(ERR,
			"%s: invalid BPGEN MMAP parameters", name);
		return -1;
	}

	PMD_LOG(INFO, "%s: BPGEN MMAP parameters:", name);
	...
	eth_dev->rx_pkt_burst = eth_bpgen_rx;
	eth_dev->tx_pkt_burst = eth_bpgen_tx;
...
static int
rte_pmd_bpgen_probe(struct rte_vdev_device *dev)
{
	int ret = 0;
	struct rte_kvargs *kvlist;
	int sockfd = -1;
	...
	PMD_LOG(INFO, "Initializing pmd_bpgen for %s", name);
	 ...
	 * reading / writing
	 */
	if (rte_kvargs_count(kvlist, ETH_BPGEN_IFACE_ARG) == 1) {

		ret = rte_kvargs_process(kvlist, ETH_BPGEN_IFACE_ARG,
...
static int
rte_pmd_bpgen_remove(struct rte_vdev_device *dev)
{
	struct rte_eth_dev *eth_dev = NULL;
	struct pmd_internals *internals;
	unsigned q;

	PMD_LOG(INFO, "Closing BPGEN ethdev on numa socket %u",
		rte_socket_id());
...
static struct rte_vdev_driver pmd_bpgen_drv = {
	.probe = rte_pmd_bpgen_probe,
	.remove = rte_pmd_bpgen_remove,
};

RTE_PMD_REGISTER_VDEV(net_bpgen, pmd_bpgen_drv);
RTE_PMD_REGISTER_ALIAS(net_bpgen, eth_bpgen);
RTE_PMD_REGISTER_PARAM_STRING(net_bpgen,
	"iface=<string> "
	"qpairs=<int> "
	"blocksz=<int> "
	"framesz=<int> "
...
RTE_INIT(bpgen_init_log)
{
	bpgen_logtype = rte_log_register("pmd.net.bpgen");
	if (bpgen_logtype >= 0)
		rte_log_set_level(bpgen_logtype, RTE_LOG_NOTICE);
}
